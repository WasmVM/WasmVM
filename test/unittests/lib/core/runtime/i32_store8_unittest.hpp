...
SKYPAT_F(Runtime_i32_store_8, regular)
{
    ...
    int ret = runtime_i32_store8(stack, memory, offset, 0);  // ea > 65536 check
    EXPECT_EQ(ret, -1);
...
SKYPAT_F(Runtime_i32_load_and_store_8, regular)
{
    Value   *var = NULL;
    MemInst *memory = new_MemInst();
    Stack   *stack  = new_Stack();
    ...
    for(uint32_t lop = 0; lop < 8; lop++) {
        stack->entries->push(stack->entries, new_i32Value(lop * sizeof(int32_t)));
        runtime_i32_load8_u(stack, memory, offset, 0);
