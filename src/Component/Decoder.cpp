...
void Decoder::decode(Store& store, Stack& coreStack) {
  ...
  switch (bincode) {
    ...
    case OP_Ctrl_nop:
      break;
    case OP_Ctrl_block:
      Instruction::ctrl_block(*funcBody, coreStack,
                              funcBody->at(coreStack.curLabel->instrOffset++));
      break;
    case OP_Ctrl_loop:
      Instruction::ctrl_loop(coreStack,
                             funcBody->at(coreStack.curLabel->instrOffset++));
      break;
    case OP_Ctrl_if:
      Instruction::ctrl_if(*funcBody, coreStack,
                           funcBody->at(coreStack.curLabel->instrOffset++));
      break;
    case OP_Ctrl_else:
    ...
    case OP_Ctrl_br:
      try {
        Instruction::ctrl_br(
            Util::getLeb128_u32(*funcBody, coreStack.curLabel->instrOffset),
            coreStack);
      } catch (const char* e) {
    ...
    case OP_Ctrl_call_indirect:
      try {
        Instruction::ctrl_call_indirect(
            Util::getLeb128_u32(*funcBody, coreStack.curLabel->instrOffset),
            store, coreStack);
      } catch (const char* e) {
    ...
    case OP_i32_const:
      try {
        Instruction::i32_const(
            coreStack,
            Util::getLeb128_i32(*funcBody, coreStack.curLabel->instrOffset));
      } catch (const char* e) {
    ...
    case OP_i64_const:
      try {
        Instruction::i64_const(
            coreStack,
            Util::getLeb128_i64(*funcBody, coreStack.curLabel->instrOffset));
      } catch (const char* e) {
    ...
    case OP_f32_const:
      try {
        Instruction::f32_const(
            coreStack,
            Util::getIEEE754_f32(*funcBody, coreStack.curLabel->instrOffset));
      } catch (const char* e) {
    ...
    case OP_f64_const:
      try {
        Instruction::f64_const(
            coreStack,
            Util::getIEEE754_f64(*funcBody, coreStack.curLabel->instrOffset));
      } catch (const char* e) {
...
#else
      std::cout << Exception(std::string("Code: 0x") + codeChr, coreStack).desc
                << std::endl;
#endif
    } break;
  }
