...
class Util {
 public:
  ...
  static std::uint32_t getLeb128_u32(std::vector<char>& body,
                                     std::uint64_t& offset);
  static std::int32_t getLeb128_i32(std::vector<char>& body,
                                    std::uint64_t& offset);
  static std::int64_t getLeb128_i64(std::vector<char>& body,
                                    std::uint64_t& offset);
  ...
  static std::uint64_t getContinueOffset(std::vector<char>& funcBody,
                                         Stack& coreStack,
                                         std::uint64_t instrOffset,
                                         bool stopElse = false);
  static char* getMemoryPtr(Store& store,
                            Stack& coreStack,
                            std::uint32_t align,
                            std::uint32_t offset,
                            int byteWidth,
