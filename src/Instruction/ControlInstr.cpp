...
void Instruction::invoke(std::uint32_t funcAddr,
                         Store& store,
                         Stack& coreStack,
                         ModuleInst* moduleInst) {
  if (funcAddr >= store.funcs.size()) {
    throw Exception("[invoke] Function address not exist.", coreStack,
                    moduleInst);
  }
  FuncInst* funcInst = store.funcs.at(funcAddr);
  if (coreStack.valueNum < funcInst->type.paramTypes.size()) {
    throw Exception("[invoke] No enough values for function parameters",
                    coreStack, moduleInst);
  }
  Frame newFrame;
  newFrame.moduleInst = funcInst->module;
  newFrame.locals.resize(funcInst->type.paramTypes.size() +
                         funcInst->code.localTypes.size());
  // Set params
  ...
  // Set locals
  for (size_t i = 0, j = funcInst->type.paramTypes.size();
       i < funcInst->code.localTypes.size(); ++i, ++j) {
...
void Instruction::ctrl_end(Stack& coreStack) {
  ...
  if (coreStack.valueNum != resCount) {
    if (coreStack.valueNum > resCount) {
      throw Exception(
          "[end] Too many values left in the stack while block ending.",
          coreStack);
    } else {
      throw Exception(
          "[end] Too few values left in the stack for the block result.",
          coreStack);
    }
  }
  // Pop values
  ...
  if (popElem.type != StackElemType::label) {
    throw Exception(
        "[end] There must be a label on the top of stack after popping values.",
        coreStack);
  }
...
void Instruction::ctrl_block(std::vector<char>& funcBody,
                             Stack& coreStack,
                             char blocktype) {
  Label newLabel;
  switch (blocktype) {
  ...
  try {
    newLabel.contOffset =
        Util::getContinueOffset(funcBody, coreStack, newLabel.instrOffset) + 1;
  } catch (const char* e) {
...
void Instruction::ctrl_return(Stack& coreStack) {
  ...
  if (coreStack.valueNum < resCount) {
    throw Exception(
        "[return] Too few values left in the stack for the block result.",
        coreStack);
  }
  // Pop values
  std::vector<Value> resultValues;
...
void Instruction::ctrl_call(std::uint32_t funcAddr,
                            Store& store,
                            Stack& coreStack) {
  ...
  // Invoke
  invoke(coreStack.curFrame->moduleInst->funcaddrs.at(funcAddr), store,
         coreStack);
}
void Instruction::ctrl_call_indirect(std::uint32_t typeidx,
                                     Store& store,
                                     Stack& coreStack) {
  // Bypass reserved
  ...
  if (tableAddr >= store.tables.size()) {
    throw Exception("[call_indirect] Table address not exist in the store.",
                    coreStack);
  }
  // Check function type
  if (typeidx >= coreStack.curFrame->moduleInst->types.size()) {
    throw Exception("[call_indirect] Function type not exist in this module.",
                    coreStack);
  }
  // Check operand
  ...
  // Match type
  if (store.funcs.at(funcAddr)->type !=
      coreStack.curFrame->moduleInst->types.at(typeidx)) {
...
void Instruction::ctrl_br(std::uint32_t depth, Stack& coreStack) {
  ...
  for (std::list<StackElem>::iterator stackIt = coreStack.begin();
       stackIt != coreStack.end(); ++stackIt) {
  ...
  if (coreStack.valueNum < resTypes.size()) {
    throw Exception(
        "[br] Too few values left in the stack for target block result.",
        coreStack);
  }
  // Pop values
  std::vector<Value> resultValues;
  ...
  for (size_t i = 0; i < depth + 1; ++i) {
    ...
    if (coreStack.top().type != label) {
      throw Exception("[br] Stack top must be label after popping values.",
                      coreStack);
    } else {
...
void Instruction::ctrl_br_table(std::vector<std::uint32_t>& depths,
                                Stack& coreStack) {
  // Check stack
  if (coreStack.valueNum < 1) {
...
void Instruction::ctrl_if(std::vector<char>& funcBody,
                          Stack& coreStack,
                          char blocktype) {
  // Check stack
  if (coreStack.valueNum < 1) {
  ...
  if (val->data.i32) {
    ...
    try {
      newLabel.contOffset =
          Util::getContinueOffset(funcBody, coreStack, newLabel.instrOffset) +
          1;
    } catch (const char* e) {
    ...
    try {
      newLabel.instrOffset = Util::getContinueOffset(
          funcBody, coreStack, coreStack.curLabel->instrOffset, true);
      newLabel.contOffset = Util::getContinueOffset(funcBody, coreStack,
                                                    newLabel.instrOffset - 1) +
                            1;
    } catch (const char* e) {
      throw Exception(std::string("[if] ") + e, coreStack);
    }
...
void Instruction::ctrl_else(Stack& coreStack) {
  ...
  if (coreStack.valueNum != resCount) {
    throw Exception(
        "[else] Values left in the stack not the correct number for result.",
        coreStack);
  }
  // Pop values
  std::vector<Value> resultValues;
  ...
  if (popElem.type != StackElemType::label) {
    throw Exception(
        "[else] There must be a label on the top of stack after pop result "
        "value.",
        coreStack);
  }
  // Return from function
