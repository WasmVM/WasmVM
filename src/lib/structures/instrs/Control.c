...
WasmControlInstr* new_WasmControlInstr()
{
    ...
    newInstr->resultTypes = new_vector_p(sizeof(ValueType), NULL);
    newInstr->instrs = new_vector_p(sizeof(ValueType), NULL);
    newInstr->indices = new_vector_p(sizeof(ValueType), NULL);
    return newInstr;
}
