...
int runtime_f64_div(Stack stack)
{
    ...
    } else if((isinf(operand1->value.f64) && isinf(operand2->value.f64)) || (operand1->value.f64 == 0 && operand2->value.f64 == 0)) {
        push_Value(stack, new_f64Value(nan("")));
    } else if(isinf(operand1->value.f64) || operand2->value.f64 == 0) {
        if(signbit(operand1->value.f64) ^ signbit(operand2->value.f64)) {
            push_Value(stack, new_f64Value(-strtod("INF", NULL)));
        } else {
            push_Value(stack, new_f64Value(+strtod("INF", NULL)));
        }
    } else if(operand2->value.f64 == 0 || isinf(operand1->value.f64)) {
        if(signbit(operand1->value.f64) ^ signbit(operand2->value.f64)) {
