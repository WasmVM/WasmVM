...
int runtime_f32_add(Stack stack)
{
    ...
    if(isnan(operand1->value.f32) || isnan(operand2->value.f32)) {
        ...
        if(isinf(operand1->value.f32) && isinf(operand2->value.f32) && (signbit(operand1->value.f32) ^ signbit(operand2->value.f32))) {
            push_Value(stack, new_f32Value(nanf("")));
        } else if((isinf(operand1->value.f32) && !signbit(operand1->value.f32)) || (isinf(operand2->value.f32) && !signbit(operand2->value.f32))) {
        ...
        if(signbit(operand1->value.f32) ^ signbit(operand2->value.f32)) {
            ...
            push_Value(stack, new_f32Value(operand1->value.f32));
        }
    } else {
