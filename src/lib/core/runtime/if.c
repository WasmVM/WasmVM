...
int runtime_if(Stack *stack, ControlInstrInst *control)
{
    ...
    if(value->value.i32 == 0) {
        label = new_Label(label_get_funcAddr(stack->curLabel), control->elseAddr + 1, control->endAddr);
    } else {
        label = new_Label(label_get_funcAddr(stack->curLabel), label_get_instrIndex(stack->curLabel) + 1, control->endAddr);
    }
