...
int runtime_f32_div(Stack stack)
{
    ...
    } else if((isinf(operand1->value.f32) && isinf(operand2->value.f32)) || (operand1->value.f32 == 0 && operand2->value.f32 == 0)) {
        push_Value(stack, new_f32Value(nanf("")));
    } else if(isinf(operand1->value.f32) || operand2->value.f32 == 0) {
        if(signbit(operand1->value.f32) ^ signbit(operand2->value.f32)) {
            push_Value(stack, new_f32Value(-strtof("INF", NULL)));
        } else {
            push_Value(stack, new_f32Value(+strtof("INF", NULL)));
        }
    } else if(operand2->value.f32 == 0 || isinf(operand1->value.f32)) {
        if(signbit(operand1->value.f32) ^ signbit(operand2->value.f32)) {
