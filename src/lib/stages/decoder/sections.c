...
int parse_export_section(WasmModule *newModule, uint8_t **read_p, uint8_t **end_p)
{
    ...
    while(exportNum-- > 0) {
        ...
        *read_p += nameLen;
        newExport->name = name;
        // FIXME: add checking mechanism, to pop out newModule->exports
        // , and compare with name with newExport->name
...
int parse_start_section(WasmModule *newModule, uint8_t **read_p, uint8_t **end_p)
{
    ...
    // FIXME: checking mechanism
    // - index in Funcs: use newModule->start - importedFuncCount
    // - checking if start function is a void function
...
int parse_code_section(WasmModule *newModule, uint8_t **read_p, uint8_t **end_p)
{
    ...
    for(uint32_t i = 0; i < codeNum; ++i) {
        ...
        strncpy(existedInstr->opcode, *read_p, codeSize);
        // shift
        *read_p += codeSize;
    }
...
int parse_data_section(WasmModule *newModule, uint8_t **read_p, uint8_t **end_p)
{
    ...
    while(dataNum-- > 0) {
        ...
        *read_p++;
        ...
        strncpy(initData, *read_p, dataSize);
