...
Context* new_Context(WasmModule* module, WasmFunc* func)
{
    ...
    context->locals = new_vector_p(sizeof(ValueType), NULL);
    // params
    ...
    context->returns = new_vector_p(sizeof(ValueType), NULL);
    // returns
...
ctrl_frame* new_ctrl_frame(stack_p opds)
{
    ...
    frame->unreachable = 0;
    frame->label_types = new_vector_p(sizeof(ValueType), NULL);
    frame->end_types = new_vector_p(sizeof(ValueType), NULL);
